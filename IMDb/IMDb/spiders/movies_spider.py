...
class MoviesSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        movies = response.css('li.ipc-metadata-list-summary-item sc-10233bc-0 iherUv cli-parent')
        for movie in movies:
            yield{
                'name' : movie.css('div.ipc-metadata-list-summary-item__c div div div a.ipc-title-link-wrapper h3::text').get(),
                # 'price' : movie.css('.product_price .price_color::text').get(),
                # 'url' : movie.css('h3 a::attr(href)').get(),
            }
